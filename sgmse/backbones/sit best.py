...
class SiT(nn.Module):
    ...
    def forward(self, x, t):
        ...
        # Convert real and imaginary parts into four channel dimensions
        x = torch.cat((x[:, [0], :, :].real, x[:, [0], :, :].imag,
                       x[:, [1], :, :].real, x[:, [1], :, :].imag), dim=1)
        
        # # x = 2 * x - 1.0  # Scale input to [-1, 1]
        ...
        # 转换回复数形式
        x = torch.permute(x, (0, 2, 3, 1)).contiguous()  # (N, H, W, 2)
        x = torch.view_as_complex(x)[:, None, :, :]  # (N, H, W), complex
