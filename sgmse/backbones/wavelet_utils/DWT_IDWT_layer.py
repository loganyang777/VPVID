...
class DWT_1D(Module):
    ...
    def get_matrix(self):
        ...
        L = math.floor(L1 / 2)
        # 计算最大可能的索引位置
        max_index_h = 2 * (L - 1) + self.band_length
        max_index_g = 2 * (L1 - L - 1) + self.band_length
        second_dim = max(L1 + self.band_length - 2, max(max_index_h, max_index_g))
        
        # 使用计算出的更大尺寸创建矩阵
        matrix_h = np.zeros((L, second_dim))
        matrix_g = np.zeros((L1 - L, second_dim))
