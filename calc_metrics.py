...
from torchaudio import load as read
...
if __name__ == '__main__':
    ...
    for  noisy_file in tqdm(noisy_files):
        if args.txt:
            ...
            x, sr_x = read(join(clean_dir, filename))
            y, sr_y = read(noisy_file)
            x_method, sr_x_method = read(join(enhanced_dir, filename))
            if sr_x != sr:
                resampler = Resample(orig_freq=sr_x, new_freq=sr)
                x = resampler(x)
            if sr_y != sr:
                resampler = Resample(orig_freq=sr_y, new_freq=sr)
                y = resampler(y)
            if sr_x_method != sr:
                resampler = Resample(orig_freq=sr_x_method, new_freq=sr)
                x_method = resampler(x_method)
            x_method = x_method.squeeze().numpy()
            x = x.squeeze().numpy()
            y = y.squeeze().numpy()
        n = y - x 
        if compare_to_real:
    ...
    # python calc_metrics.py --test_dir ./data_reverb/test_t60/t60_0.2/anechoic --enhanced_dir ./data_reverb/output/t60_0.2/vise_wavelet_sde